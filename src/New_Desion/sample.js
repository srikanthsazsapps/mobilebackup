...
const VehicleDetailsScreen = () => {
  ...
  // Reset animation on mount and unmount
  useEffect(() => {
  ...
  const onPanHandlerStateChange = (event) => {
    if (event.nativeEvent.oldState === State.ACTIVE) {
      ...
      if (currentPosition < 0) {
        toValue = gestureTranslationY > -50 || velocityY > 200 ? 0 : -205;
      } else {
  ...
  const handleViewMore = () => {
    setOpenPopupType('Highly Used');
  };

  const statusCounts = getStatusCounts();
  const maxValue = Math.max(...Object.values(statusCounts));

  // Animation for AssetBarChart visibility
  ...
  // HighlyUsedVehicleCard Component
  const HighlyUsedVehicleCard = ({ onViewMore }) => {
    const { assetData } = useContext(DashesDataContext);

    const tripCount = assetData
      ? assetData.reduce((sum, v) => sum + (parseInt(v.TripCount) || 0), 0)
      : 0;

    const hoursCount = assetData
      ? assetData.reduce((sum, v) => sum + (parseFloat(v.TotalHours || v.Totalhours) || 0), 0)
      : 0;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        ...
        <View style={styles.chartWrapper}>
          <View style={styles.chartContainer}>
            {Object.entries(statusCounts).map(([label, count]) => {
              const minBarHeight = 50,
                maxBarHeight = 125;
              const barHeight =
                maxValue > 0 ? minBarHeight + (count / maxValue) * (maxBarHeight - minBarHeight) : minBarHeight;
          ...
          <HighlyUsedVehicleCard onViewMore={handleViewMore} />
        </View>
...
const styles = StyleSheet.create({
  ...
  chartWrapper: {
    ...
    minHeight: 200, // Ensure enough height for chartLineContainer
  },
  chartContainer: {
    flexDirection: 'row',
    justifyContent: 'space-around',
    ...
    zIndex: 1, // Ensure chartContainer is below chartLineContainer
  },
  chartLineContainer: {
    position: 'absolute',
